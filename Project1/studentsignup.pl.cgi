...
sub CheckForEmail() {
	...
	open (FH, "studentpasswordlist.txt") || die "Died opening studentpasswordlist.txt";		
	my $found=0;
...
sub CreateAccount() 
	...
	my $user = $_[0];
	my $pass = md5_hex($_[1]);
	my $output = "studentpasswordlist.txt";
	...
	my $calName = $user;
	$calName =~ s/@//;
	my $filename = "STUDENTCAL". $calName . ".txt";
	open nFH, ">$filename";
	print nFH "$calName\n";
	close nFH;

	#add to BIGStudentList
	open(GH, ">>BIGStudentList.txt") || die("BIGStudentList dead");
	print GH "$user\n";
	close GH;
...
sub SendMail() 
	...
	my $subject='Student Account Confirmation for InstraSchedule';
	open(MAIL, "|/usr/sbin/sendmail -t");
	print MAIL "To: $to\n";
	print MAIL "From: $from\n";
	print MAIL "Subject: $subject\n\n";
	print MAIL "Thank you for creating a student account with InstraSchedule.";
	close(MAIL);
}
...
<body id = "type-a">
	...
	<a href="./studentlogin.pl.cgi"> Log In With Your New Account </a>
	</div></div></div>
</body></html>
EndOfPrinting
}
...
<body id = "type-a">
	...
	<div id = "content">
	<h1> Sign Up Below </h1>
	<FORM ACTION="./studentsignup.pl.cgi" METHOD="POST">
